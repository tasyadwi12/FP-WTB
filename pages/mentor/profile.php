...
try {
    $stats_result = query("SELECT 
        COUNT(DISTINCT ms.siswa_id) as total_siswa,
        COUNT(DISTINCT pm.progress_id) as total_penilaian,
        AVG(pm.nilai) as avg_nilai
        FROM mentor_siswa ms
        LEFT JOIN progress_materi prog ON ms.siswa_id = prog.user_id
        LEFT JOIN penilaian_mentor pm ON prog.progress_id = pm.progress_id AND pm.mentor_id = :mentor_id
        WHERE ms.mentor_id = :mentor_id2", [
        'mentor_id' => $user_id,
        'mentor_id2' => $user_id
    ]);
    
    $mentoring_stats = $stats_result ? $stats_result->fetch() : null;
} catch (Exception $e) {
    $mentoring_stats = null;
}

$stats = [
    'total_siswa' => $mentoring_stats['total_siswa'] ?? 0,
    'total_penilaian' => $mentoring_stats['total_penilaian'] ?? 0,
    'avg_nilai' => round($mentoring_stats['avg_nilai'] ?? 0, 1)
];
            ...
            <div class="col-md-4 mb-4">
                ...
                <div class="card border-0 shadow-sm mt-3">
                    <div class="card-header bg-white">
                        <h6 class="mb-0"><i class="fas fa-chart-bar me-2"></i>Statistik Bimbingan</h6>
                    </div>
                    <div class="card-body">
                        <div class="row text-center">
                            <div class="col-12 mb-3">
                                <div class="bg-primary bg-opacity-10 rounded p-3">
                                    <h3 class="text-primary mb-1"><?php echo $stats['total_siswa']; ?></h3>
                                    <small class="text-muted">Total Siswa</small>
                                </div>
                            </div>
                            <div class="col-6 mb-3">
                                <div class="bg-success bg-opacity-10 rounded p-3">
                                    <h4 class="text-success mb-1"><?php echo $stats['total_penilaian']; ?></h4>
                                    <small class="text-muted">Penilaian</small>
                                </div>
                            </div>
                            <div class="col-6 mb-3">
                                <div class="bg-info bg-opacity-10 rounded p-3">
                                    <h4 class="text-info mb-1"><?php echo $stats['avg_nilai']; ?></h4>
